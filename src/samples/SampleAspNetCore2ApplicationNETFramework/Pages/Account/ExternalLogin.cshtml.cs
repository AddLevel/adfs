    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        class Saml2ClaimsFactory : IUserClaimsPrincipalFactory<ApplicationUser>
        {
            ...
            public async Task<ClaimsPrincipal> CreateAsync(ApplicationUser user)
            {
                ...
                //NOTE: We need custom claims to the application...
                var roles = _externalLoginInfo.Principal.FindAll("http://schemas.microsoft.com/ws/2008/06/identity/claims/role");
                if (roles != null)
                {
                    identity.AddClaims(roles);
                }

                var groups = _externalLoginInfo.Principal.FindAll("groups");
                if (groups != null)
                {
                    identity.AddClaims(groups);
                }
